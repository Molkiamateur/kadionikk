...
int main(int argc, char* argv[])
{
    ...
    for(int buffSize = v_begin; buffSize <= v_end; buffSize *= v_step)
    {
...
#endif
#ifdef __AVX512F__
        float* o_avx512 = new float[buffSize];
#endif
        ...
        //
        // PROCEDURE DE TEST SUR LES CODES AVX512
        //

#ifdef __AVX512F__
        auto start_i_avx512 = std::chrono::system_clock::now();
        for(int32_t loop = 0; loop < nTest; loop += 1)
            vec_add_avx512(o_avx512, a_tab, b_tab, buffSize);
        auto stop_i_avx512 = std::chrono::system_clock::now();
#endif
...
#endif
#ifdef __AVX512F__
            const uint64_t time_avx512 = std::chrono::duration_cast<std::chrono::nanoseconds>(stop_i_avx512 - start_i_avx512).count() / nTest;
#endif
...
#endif
#ifdef __AVX512F__
        if( is_ok(o_x86, o_avx512, buffSize) ){
            printf("    - [AVX-512] hand-written  \033[32mOK\033[0m [%5d ns]\n", (int32_t)time_avx512);
        }else{
            printf("    - [AVX-512] hand-written  \033[31mKO\033[0m [%5d ns]\n", (int32_t)time_avx512);
            return EXIT_FAILURE;
        }
#endif
        printf("\n");
