...
#include "tools.hpp"
#include "vmax/x86/vec_max_x86.hpp"
#include "vmax/neon/vec_max_neon.hpp"
#include "vmax/sse4/vec_max_sse4.hpp"
#include "vmax/avx2/vec_max_avx2.hpp"
...
int main(int argc, char* argv[])
{
    ...
    printf("(II) Stage 2 - Let's try with a more complex example ;-) on MacOS\n");
#elif defined (__linux__)
    printf("(II) Stage 2 - Let's try with a more complex example ;-) on Linux\n");
#else
    printf("(II) Stage 2 - Let's try with a more complex example ;-) on a undefined platform\n");
#endif
    ...
    {
        int disp_size = ((int)sizeof(float) * buffSize);
             if( disp_size <      1024 ) printf("+> memory size [%4d  B]\n", (disp_size) );
        else if( disp_size < 1024*1024 ) printf("+> memory size [%4d kB]\n", (disp_size) / 1024 );
        else                             printf("+> memory size [%4d MB]\n", (disp_size) / 1024 / 1024 );

        int nTest   = (2048*1024/buffSize)*1024;
        ...
        for(int loop = 0; loop < nTest; loop += 1)
            r_nov += vec_max_x86_novect(a_tab, buffSize);
        ...
        float r_x86 = 0.0f;
        for(int loop = 0; loop < nTest; loop += 1)
            r_x86 += vec_max_x86(a_tab, buffSize);
        ...
        float r_sse4 = 0.0f;
        for(int loop = 0; loop < nTest; loop += 1)
            r_sse4 += vec_max_sse4(a_tab, buffSize);
        ...
        float r_avx2 = 0.0f;
        for(int loop = 0; loop < nTest; loop += 1)
            r_avx2 += vec_max_avx2(a_tab, buffSize);
        ...
        float r_neon = 0.0f;
        for(int loop = 0; loop < nTest; loop += 1)
            r_neon += vec_max_neon(a_tab, buffSize);
